...
public interface IImage extends ISerializable {
	...
	@ZenGetter("images")
	default List<String> getImagesAsString() {
		if(this.getImages().size()>1) {
			return this.getImages().stream().map(ResourceLocation::toString).collect(Collectors.toList());
		}
		return Arrays.asList(this.getImageAsString());
	}
