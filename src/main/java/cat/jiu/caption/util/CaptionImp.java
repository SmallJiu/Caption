...
public final class CaptionImp {
	...
	static final Map<CaptionType, Map<String, Caption>> currentCaptions = Maps.newHashMap();
	static final Map<CaptionType, Map<String, List<Caption>>> alternativeCaptions = Maps.newHashMap();
	static final Map<CaptionType, Map<String, Caption>> lastCurrentCaptions = Maps.newHashMap();
	
	public static void addCaption(EntityPlayer player, Caption caption) {
		addCaption(player.getName(), caption);
	}
	static void addCaption(String name, Caption caption) {
	...
	public static Caption getCurrentCaption(CaptionType type) {return getCurrentCaption(type, Minecraft.getMinecraft().player.getName());}
	@SideOnly(Side.CLIENT)
	...
	@SideOnly(Side.CLIENT)
	public static Caption getNextCaption(CaptionType type) {return getNextCaption(type, Minecraft.getMinecraft().player.getName());}
	@SideOnly(Side.CLIENT)
	...
	public static Caption getCurrentCaption(CaptionType type, String name) {
		if(!currentCaptions.containsKey(type)) return null;
		return currentCaptions.get(type).get(name);
	}
	public static Caption getNextCaption(CaptionType type, String name) {
	...
	public static boolean hasNextCaption(CaptionType type, String name) {
		...
		List<Caption> e = alternativeCaptions.get(type).get(name);
		return e!=null && !e.isEmpty();
	}
	public static Caption getLastCaption(CaptionType type, String name) {
		...
		new Thread(()->{
			while(true) {
				...
				if(!alternativeCaptions.isEmpty()) {
					for(Entry<CaptionType, Map<String, List<Caption>>> currents : Sets.newHashSet(alternativeCaptions.entrySet())) {
						Map<String, List<Caption>> values = alternativeCaptions.get(currents.getKey());
						if(values==null || values.isEmpty()) {
							continue;
						}
						for(Entry<String, List<Caption>> elements : Sets.newHashSet(values.entrySet())) {
							List<Caption> element = values.get(elements.getKey());
				...
				if(!currentCaptions.isEmpty()) {
					for(Entry<CaptionType, Map<String, Caption>> allCurrents : currentCaptions.entrySet()) {
						for(Entry<String, Caption> currents : allCurrents.getValue().entrySet()) {
							try {
	...
	public static void onPlayerLeaveServer(PlayerLoggedOutEvent event) {
		for(Entry<CaptionType, Map<String, Caption>> currents : Sets.newHashSet(CaptionImp.currentCaptions.entrySet())) {
			CaptionImp.currentCaptions.get(currents.getKey()).remove(event.player.getName());
		}
		for(Entry<CaptionType, Map<String, List<Caption>>> currents : Sets.newHashSet(CaptionImp.alternativeCaptions.entrySet())) {
		...
		if(event.phase == Phase.START) {
			...
			for(CaptionType type : CaptionType.VALUES) {
				if(CaptionImp.hasCurrentCaption(type, name)) {
					Caption current = CaptionImp.getCurrentCaption(type, name);
	...
	private static void next(String playerName) {
		for(Entry<CaptionType, Map<String, List<Caption>>> currents : CaptionImp.alternativeCaptions.entrySet()) {
			CaptionType type = currents.getKey();
			
			if(CaptionImp.hasNextCaption(type, playerName)) {
				List<Caption> elements = currents.getValue().get(playerName);
	...
	public static void onRenderDebugInfo(RenderGameOverlayEvent.Text event) {
		if(Minecraft.getMinecraft().gameSettings.showDebugInfo) {
			...
			if(CaptionImp.hasCurrentCaption(CaptionType.Main)) {
				Caption current = CaptionImp.getCurrentCaption(CaptionType.Main);
		...
		if(event.getType() == ElementType.CHAT && !Minecraft.getMinecraft().gameSettings.hideGUI) {
			...
			for(CaptionType type : CaptionType.VALUES) {
				if(CaptionImp.hasCurrentCaption(type)) {
					Caption current = CaptionImp.getCurrentCaption(type);
